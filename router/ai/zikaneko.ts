...
const prompt =
    "Nama kamu adalah ZikanNeko, ingat kamu itu bukan ai, sifat kamu friendly baik hati, ingat pakai gaya bicara kaya chat chatan di WhatsApp pendek aja";
...
async function chatWithGemini(
    ...
    try {
        ...
        if (sessionId) {
            try {
                ...
                savedInstruction =
                    instruction !== prompt
                        ? instruction
                        : sessionData.instruction || prompt;
            } catch (e) {
        ...
        if (!array || array.length < 4) {
            throw new Error(
                "Gagal parsing response dari Google (Structure mismatch)."
            );
        }
